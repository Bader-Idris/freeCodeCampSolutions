...
PSQL="psql -X -U postgres -p 5433 -d bikes --tuples-only -c"
echo -e "\n~~~~~ Bike Rental Shop ~~~~~\n"
...
RENT_MENU() {
  ...
  if [[ -z $AVAILABLE_BIKES ]] ; then
    ...
    do
      #* it has bugs return psql code, before mapped ones
      #*  it's because of the way using while with psql
      #* using IFS='|' with removing the BAR might be useful
    ...
    if [[ ! $BIKE_ID_TO_RENT =~ ^[0-9]+$ ]]; then
      ...
      if [[ -z $BIKE_AVAILABILITY ]]; then
        ...
        if [[ -z $CUSTOMER_NAME ]]; then
          #* it's gonna be empty, when no results are found!
        ...
        BIKE_INFO_FORMATTED=$(echo $BIKE_INFO | sed 's/ |/"/')
          #! echo '28 | Mountain' | sed 's/ |//' solves the printing issue above!
        # send to main menu
        MAIN_MENU "I have put you down for the $BIKE_INFO_FORMATTED Bike, $(echo $CUSTOMER_NAME | sed -r 's/^ *| *$//g')."
        #? they insist of using: 's/^ *| *$//g' instead of 's/^ *|*$//g'
      fi
      # send to main menu
    fi
  fi
